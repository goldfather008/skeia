...
(function($){
	...
	$.extend({
	    ...
	    */
	    ...
	    /**
		 * 1、设置控件状态（新增、修改）
		 * controlsId: 控件id,多个控件可以通过逗号分隔
		 * state:可以设置disable或者enable或者readonly；
		 * 
		 * 
		 */
		disableControls : function(controlsId,state) {
			if(controlsId==""||controlsId.split(",").length<=0){
				alert("请设置控件ID");
				return;
			}
			if(state!="disable"&&state!="enable"&&state!="readonly"){
				alert("请设置控件是否有效状态");
				return;
			}	
			var controls=controlsId.split(",");
		   for(var key in controls){  
			   if($("#"+controls[key]).is("input")){
				  // if(""!=$("#"+controls[key]).val()&&"0"!=$("#"+controls[key]).val()&&"0.0"!=$("#"+controls[key]).val())
				   $("#"+controls[key]).css({"background":"transparent","border":"1px","solid":"#ffffff"}).attr("readonly","readonly").removeAttr("onclick").removeAttr("class"); 
			   }
			   if($("#"+controls[key]).is("select")){
				   if(state=="readonly"){
					//   if(""!=$("#"+controls[key]).val()&&"0"!=$("#"+controls[key]).val())
					   $("#"+controls[key]+" option:not(:selected)").attr("disabled","disabled");
				   }
			   } 		   
		   }  
		 },
		...
		/**
		 * 1、设置控件状态（新增、修改）
		 * controlsName: 控件Name,多个控件可以通过逗号分隔
		 * state:可以设置disable或者enable或者readonly；
		 * 
		 */
		disableForCheackBoxOrRedio : function(controlsName,state) {
			if(controlsName==""||controlsName.split(",").length<=0){
				alert("请设置控件ID");
				return;
			}
			if(state!="disable"&&state!="enable"&&state!="readonly"){
			...
			var controls=controlsName.split(",");
			
		   for(var key in controls){ 
			   if($("input[name='"+controls[key]+"']").is(":checkbox")){
				   if($("input[name='"+controls[key]+"']:checked").length>0){
				   $("input[name='"+controls[key]+"']").css("color","#848484");
				   $("input[name='"+controls[key]+"']").removeAttr("onclick");
				   $("input[name='"+controls[key]+"']").click(function(){
					   this.checked = !this.checked;
					 });
				   }
			   }else if($("input[name='"+controls[key]+"']").is(":radio")){
				   if(null!=$("input[name='"+controls[key]+"']:checked").val()&&""!=$("input[name='"+controls[key]+"']:checked").val()){
					   $("input[name='"+controls[key]+"']").css("color","#848484");
					   $("input[name='"+controls[key]+"']").removeAttr("onclick");
					   $("input[name='"+controls[key]+"']").not($("input[name='"+controls[key]+"'][checked]")).attr("disabled","disabled");
					}
			}
		   }  
		 }
